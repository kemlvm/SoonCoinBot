...
def find_resistance(data):
    ...
    histogram = np.histogram(maximums, bins=(
        int(len(maximums)/increment*increment)))
    ...
    for i in histogram_bins.index:
        lst_maxser.append(
            data[(data > histogram_bins['start'][i]) & (data < histogram_bins['end'][i])])
...
def find_support(data):
    ...
    histogram = np.histogram(minimums, bins=(
        int(len(minimums)/increment*increment)))
    histogram_occurences = pd.DataFrame(histogram[0])
    histogram_occurences.columns = ['occurence']
    histogram_splits = pd.DataFrame(histogram[1])
    histogram_splits.columns = ['bins']
...
result = gentrends(x, window=1.0/3, charts=False)

# Trendler hesaplıyoruz charts = False ile kendi kütüphanesindeki chart kullanılmaması sağlanıyor
